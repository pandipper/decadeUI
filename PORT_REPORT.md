# horizontal → 十周年UI 源码移植报告

## 目标
把 horizontal 对十周年UI 运行时的改动，从"直接改压缩产物"迁移到"改作者源码 + vite build 产出"，以支持长期跟版合并。
...
- **11 个新文件**：layoutEditor.js、portraitLetterbox.js、player7.css、horizontal.js(3 皮肤)、horizontal.css(5)
...
- **debug.js / extensionToggle.js**：采用 horizontal 可读版（eruda 位置记忆 / 开关增强）

## 构建验证
- `npm install + vite build` 成功，产出 `dist/`（结构 = 扩展目录，可直接覆盖到 `extension/十周年UI/`）
- dist vs horizontal 运行时全量比对：**152 字节一致 / 4 terser 名重排(功能等价) / 79 尺寸不同 / 0 缺失**
  - 4 名重排：config-window/styleHotkeys/animations/ui.constants（npm terser 5.50 vs pnpm-lock 锁版本差异，本机 pnpm 构建可消除）
  - 79 尺寸不同：5 个预期（horizontal 可读版 vs dist 压缩版）+ ~74 个因 `main` 源码比已装 v1.4 更新

## 重要说明：main vs v1.4
下载的是作者 `main` 分支源码，比你安装的 v1.4 release 略新（皮肤 baby/base/codename、CSS player1-6、libs 等有更新）。
- build 产物 = **main + horizontal**（含作者最新改动），非 v1.4 + horizontal
- 长期跟版应基于 `main`（推荐）
- 若要精确匹配当前 v1.4 安装：需 checkout `v1.4` tag 源码再 rebase horizontal commit
...
3. 启动游戏测试 horizontal 样式与各功能
4. 跟版流程：作者出新版 → 更新 `upstream` remote → `git merge` → 解冲突（高发：content.js/shizhounian.js/decadeLayout.css）→ rebuild

## 后续可选优化
- 把 debug.js / extensionToggle.js 的 horizontal 改动从"可读版整体替换"改写为"在作者源码上增量叠加"，减少未来合并噪音
